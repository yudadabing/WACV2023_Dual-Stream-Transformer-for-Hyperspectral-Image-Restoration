...
from BlockModule_T3 import DeepShare
...
def main():
    ...
    train_parser.add_argument("--batch_size", type=int, default=12, help="batch size, default set to 64")
    train_parser.add_argument("--UseLabeledSpectralMixUp", type=int, default=1, help="if we use gan loss, 0  for false, 1 for yes")
    ...
    train_parser.add_argument("--UseRGB", type=int, default=1, help="if we use rgb, 0  for false, 1 for yes")
    train_parser.add_argument("--epochs", type=int, default=30, help="epochs, default set to 20")
    ...
    train_parser.add_argument("--train_dir_mslabel", type=str, default='./matlab_code/Data/CAVE/factor_x4/train/labeled/', help="directory of train spectral dataset")
    train_parser.add_argument("--train_dir_msunlabel", type=str, default='./matlab_code/Data/CAVE/factor_x4/train/unlabeled/',
                              help="directory of train spectral dataset")
    train_parser.add_argument("--eval_dir_ms", type=str, default='./matlab_code/Data/CAVE/factor_x4/test/', help="directory of evaluation spectral dataset")
    train_parser.add_argument("--test_dir", type=str, default='./Data/CAVE/factor_x4/test/', help="directory of test spectral dataset")
    train_parser.add_argument("--train_dir_rgb", type=str, default='./matlab_code/Data/Div2k/Div2k_and_Flickr2k_train_data/', help="directory of train rgb dataset")
    train_parser.add_argument("--theta_rgb", type=int, default=30, help="how many times of rgb images regard to ms images")   ###24
    ...
    train_parser.add_argument("--data_train_num", type=int, default=4500, help="how many .mat files used in each epoch")
    train_parser.add_argument("--data_eval_num", type=int, default=12,help="how many .mat files used in each epoch")
    train_parser.add_argument("--data_test_num", type=int, default=12, help="how many .mat files used in each epoch")
    train_parser.add_argument("--model_title", type=str, default="DeepShare", help="model_title, default set to model_title")
    ...
    test_parser.add_argument("--test_dir", type=str, default='./Data/CAVE/factor_x4/test/', help="directory of test spectral dataset")
    test_parser.add_argument("--model_dir", type=str, default='./Cave_Cave_Cave_DeepShare_Blocks=3_Subs8_Ovls2_Feats=256_ckpt_epoch_13.pth', help="directory of trained model")
    ...
    test_parser.add_argument("--data_test_num", type=int, default=12, help="how many .mat files used in each epoch")
...
def train(args):
    ...
    for e in range(start_epoch, args.epochs):
        ...
        iteration = 0
        train_mslabel_loader = DataLoader(train_mslabel_set, batch_size=args.batch_size, num_workers=8, shuffle=True)
        train_msunlabel_loader = DataLoader(train_msunlabel_set, batch_size=args.theta_unlabel * args.batch_size, num_workers=8, shuffle=True)
    ...
    with torch.no_grad():
        ...
        for i, (x, lms, gt) in enumerate(test_loader):
            x, lms, gt = x.to(device), lms.to(device), gt.to(device)
            img_size=x.shape[2:4]
            y = net(x, lms, modality="spectral",img_size=img_size)
...
def save_checkpoint(args, model, epoch):
    device = torch.device("cuda" if args.cuda else "cpu")
    model.eval().cpu()
    checkpoint_model_dir = './checkpoints36'
